...
Function Main 
{
    ...
    foreach ($fac in $facilities) 
    {
        ...
        $users = Get-ADUser -SearchBase $fac.DistinguishedName -Filter * -Property Info,DisplayName,title,whenCreated,Enabled,employeeId,office,wWWHomePage | Select Name, SamAccountName, title,Info, whenCreated, Enabled, employeeID,office,wWWHomePage
        ...
        foreach ($user in $users)
        {
            ...
            if ($user.Enabled)
            {
                ...
                {
                    #Report on this user
                    $userObj = CreatePersonObject $user.Office $user.SamAccountName $user.Name $user.title $user.whenCreated $user.employeeID $user.wWWHomePage
                    $userList += $userObj
                }
            }
        }
    ...
    #Production 
    Send-MailMessage -BodyAsHtml $body -From "<svcusrprov@lcca.com>" -Subject "UP Process Verification - AD Accounts Missing Employee ID" -SmtpServer lccarelay.lcca.net `
       -To "UserProvisioningNotifications <_31f59f@lcca.com>" `
       -attachment $fileName
    #-----------------------------------------------------------------------------------------

    #Testing
    #Send-MailMessage -BodyAsHtml $body -From "<svcusrprovg@lcca.com>" -Subject "TEST - UP Process Verification - AD Accounts Missing Employee ID - TEST" -SmtpServer lccarelay.lcca.net `
...
{
    param ($facility, $SamAccountName, $name, $title, $whenCreated, $employeeId, $wWWHomePage)
    ...
    $perObj | add-member -type NoteProperty -Name WebPageAttribute -Value $wWWHomePage
