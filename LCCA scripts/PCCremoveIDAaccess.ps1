...
function RemoveIDAaccess
#-----------------------
{
    $updates = @()
    $roleChanges = @()
    #$CURRENTDATE=[DateTime]::today
    #$CURRENTDATE = Get-Date -Month 3 -day 6 -year 2018
    $facIds = GetFacilities $CURRENTDATE
    $change = CreateRoleChangeObject "IDA_Nurse_UnitManager" ""
    $roleChanges += $change
...
# Function to log and output information
{
    ...
    $filename = "\\fs3.lcca.net\edrive\MIRTH\conf\Files\PCC\Console\Logs\PCCremoveIDAaccess_" + $a + ".log"
...
$startMsg = "PCC Remove IDA Access process started on " + $startDate + " at " + $startTime
...
$filename = "\\fs3.lcca.net\edrive\MIRTH\conf\Files\PCC\Console\Output\Task\ActiveDirectory\PCCRoleMapping\PCCremoveIDAaccesss_" + $a + ".csv"

$updateList | export-csv -Path $filename

$endDate =  (Get-Date).ToShortDateString()
$endTime = (Get-Date).ToLongTimeString()
$endMsg = "PCC Remove IDA Access process Ended on " + $endDate + " at " + $endTime
