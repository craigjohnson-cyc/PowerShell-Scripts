...
Function Main
{
    DisableSurveyorsAtOldFac $newFac $oldFac
    CreateSurveyorsAtNewFac $newFac $oldFac
    
    #TODO: Call SP to link up AD and PCC accounts
    $sqlcommand = "EXEC GetFacForPS $oldFac $newFac"  #Once the SP is created, change GetFacForPS to correct name
    Invoke-SQL -dataSource "QARCPSDB1X" -database "Mirth_Repository_Dev" -sqlCommand $sqlcommand
}
...
Function DisableSurveyorsAtOldFac
{
    ...
    for($kounter=0;$kounter -lt 15; $kounter++)
    {
        ...
        #Write Log File
    $fileName = "c:\ps\output\StateSurveyorAccountsDisabledCHOW_" +  $newFac.ToString().PadLeft(4,"0") + "to"+ $oldFac.ToString().PadLeft(4,"0") +".csv"
...
Function CreateSurveyorsAtNewFac
{
    ...
    #Write Log File
    $fileName = "c:\ps\output\StateSurveyorAccountsCreatedCHOW_" +  $newFac.ToString().PadLeft(4,"0") + "to"+ $oldFac.ToString().PadLeft(4,"0") +".csv"
