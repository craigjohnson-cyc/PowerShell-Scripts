...
# •	For each facility, create 15 State Surveyor AD accounts
  ...
  WHERE Entity NOT LIKE '%(old)%' AND Level = 4
  ORDER BY LocationId "
...
function CreateADaccounts
{
    ...
    Add-ADGroupMember -Identity PCC_Surveyor -Members $user.samAccountName
    Add-ADGroupMember -Identity Cleveland -Members $user.samAccountName
}
