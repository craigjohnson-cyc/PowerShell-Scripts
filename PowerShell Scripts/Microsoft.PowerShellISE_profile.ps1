 ...
 [CmdletBinding()]
         ...
         function __findPath ([string]$currentGroup, [string]$comment) {
             ...
             if (!$UserGroups.ContainsKey($currentGroup)) {
                 ...
                 $groupObject = if ($ADGroupCache.ContainsKey($currentGroup)) {
                     ...
                     $g = Get-ADGroup -Server LCDEV.Net -Identity $currentGroup -Property objectclass,sid,whenchanged,whencreated,samaccountname,displayname,enabled,distinguishedname,memberof,groupscope,groupcategory
     ...
     process {
         ...
         foreach ($user in $UserName) {
             ...
             $UserObject = Get-ADUser -Server LCDEV.NET -Identity $user -Property objectclass,sid,whenchanged,whencreated,samaccountname,displayname,enabled,distinguishedname,memberof,PrimaryGroup
          ...
          #  $Timestamp = $Timestamp -replace("\","-")
            $OutObject | Sort-Object -Property UserName,InheritancePath,MemberOf | convertto-csv -notypeinformation |  out-file -encoding ascii -append  "C:\ps\output\LCDEV_GroupMemberships-$User-$Timestamp.csv"
            $data = Import-Csv "C:\ps\output\LCDEV_GroupMemberships-$User-$Timestamp.csv"
            $data | ogv
             $UserGroups.Clear()
...
#>
    ...
    $Employee = Get-ADuser -Server LCDEV.NET -Identity $UserName -Properties *, 'msDS-UserPasswordExpiryTimeComputed'
    $Manager = if (-not ([string]::IsNullOrEmpty($Employee.manager))) 
                    { {((Get-ADUser -Server LCDEV.NET -Identity $Employee.manager).samaccountname)}}
...
$GroupMembershipList = (Get-ADUser -Server LCDEV.NET -Filter "GivenName -like '$FirstName*' -and Surname -like '$LastName*'").SamAccountName

    Foreach ($Name in $GroupMembershipList) {

    $GroupMemberShip = Get-ADPrincipalGroupMembership -Server LCDEV.NET -Identity "$Name" | Sort Name |ForEach-Object {$_.name -replace ".*:"} 
 ...
 If ($warning -eq 'Y'){ 
  ...
  $members = Get-ADGroupMember -Server LCDEV.NET -Identity $Group 
