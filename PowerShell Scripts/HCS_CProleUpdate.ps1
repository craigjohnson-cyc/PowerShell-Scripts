...
# Read CSV
$csv = Import-Csv "C:\ps\Input\CP Descriptions and HCS Groups.csv"
...
foreach($item in $csv)
{
    $groupToAdd = $item.ADGroup
    $roleToAdd = $item.ADRole
    ...
    $users = Get-ADUser  -Filter {(description -eq $description)} -Properties entityID, employeeid, description, title, physicalDeliveryOfficeName,DistinguishedName | Select surname, givenname, title, physicalDeliveryOfficeName,description,SamAccountName,DistinguishedName
    $kount = $users.Count
    ...
    {
        # Determine if CP user
        if ($user.DistinguishedName -like '*Century Park*')
        {
