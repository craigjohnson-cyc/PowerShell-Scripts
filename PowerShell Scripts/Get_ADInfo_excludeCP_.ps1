...
Function Main
{
    ...
    # Include Century Park?
    #----------------------
    $includeCP = Read-Host -Prompt "Include Century Park Associates (Y/N)"
    ...
    Switch -Wildcard ($searchBy)
    {
        "Job*Title"
        {
            $allUsers = GetUsersByJobTitle $searchString $includeCP
        }
        "*Group"
        {
...
Function GetUserInfo
{
    ...
    if($user.enabled -eq "True")
    {
        if(![string]::IsNullOrWhiteSpace($user.office))
        {
            ...
            $facInfo = GetFacInfoFromParent $user
...
Function GetUsersByJobTitle
{
    ...
    $adQuery = "-Property initials, description, office, mail, Company, department,  title, employeeid, displayname, PhysicalDeliveryOfficeName | Select givenname, surname, initials, description, office, mail, Company, department, title, employeeid, displayname, samaccountname, enabled, DistinguishedName"
    ...
    foreach($user in $users)
    {
        if($includeCP.ToLower() -eq 'n')
        {
            if($users.DistinguishedName -like "Century Park")
            {
                continue
            }
        }
...
Function GetFacInfoFromParent
{
    param ($user)

    $userInfo = Get-ADUser $user.samaccountname | select entityid, entitytype,@{l='Parent';e={([adsi]"LDAP://$($_.DistinguishedName)").Parent.replace("LDAP://","")}} | % { Get-ADOrganizationalUnit $_.Parent -Properties entityid, entitytype }
    ...
    $query = "SELECT LongName, LocationId FROM [dbo].[OrganizationHierarchy] where LocationId = '" + $userInfo.entityid + "'"
...
Function GetFacInfo
{
    ...
    $query = "SELECT LongName, LocationId FROM [dbo].[OrganizationHierarchy] where Entity = '" + $office + "'"
    $facInfo = Invoke-SqlCmd -Query $query -ServerInstance "$ServerInstance" -Database "$Database"
    ...
    if($facInfo -eq $null)
    {
        ...
        if($facInfo -eq $null)
        {
            #Chech for () in string
            #if ($office -like "(")
            if ($office.IndexOf("(") -gt 0)
            {
                $value = $office.Substring($office.IndexOf("(")+1,$office.IndexOf(")")-$office.IndexOf("(")-1)
                $lookUp = $office.Substring(0,$office.IndexOf("(")-1).Trim()
                if($value.Length -gt 2)
                {
                    #search by city

                    $query = "SELECT LongName, LocationId FROM [dbo].[OrganizationHierarchy] where LongName like '%" + $lookUp + "%' and City Like '%" + $value + "%'"
                    $facInfo = Invoke-SqlCmd -Query $query -ServerInstance "$ServerInstance" -Database "$Database"
                }
                else
                {
                    #search by state
                    $query = "SELECT LongName, LocationId FROM [dbo].[OrganizationHierarchy] where LongName like '%" + $lookUp + "%' and State = '" + $value + "'"
                    $facInfo = Invoke-SqlCmd -Query $query -ServerInstance "$ServerInstance" -Database "$Database"
                }
            }
            else
            {
                #Test for Garden Tarrace
                if($office.IndexOf("-") -gt 0)
                {
                }
            }
